...
function dialNumber() {
  ...
  my_session.on("accepted", function () {
    uiElements.btn_call.innerHTML = 'hangUp';
    uiElements.btn_call.style.background = 'tomato';
    uiElements.btn_call.disabled = false;
      handleAccepted();
  });
  my_session.on("bye", function (request) {
    uiElements.btn_call.innerHTML = 'call';
    uiElements.btn_call.disabled = false;
    uiElements.btn_call.style.background = 'greenyellow';
    handleBye(request);
  });
  ...
  my_session.on("progress", function (progress) {
    uiElements.btn_call.innerHTML = '<span class="loading">calling</span>';
    uiElements.btn_call.style.background = 'greenyellow';
    uiElements.btn_call.disabled = true;
...
// add btn_call EventListener
uiElements.btn_call.addEventListener('click' , function(){
  uiElements.btn_call.innerHTML = '<span class="loading">wait</span>';
  uiElements.btn_call.disabled  = true;
  uiElements.digits.value       = voipiran_sid;
  dialButton(); /* dial the predefine number  */
});
