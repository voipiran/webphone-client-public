...
<body>
            ...
            <div class="bottom-container">
                <div class="bottom" v-if="isReRegister === true">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-{{ count($webphones) > 3 ? 3 : count($webphones) }} gap-4 justify-center mx-auto max-w-md">
                        ...
                        @foreach ($webphones as $webphone)
                            <div class="row btn-container">
                                <button class="btn-call" :disabled="status.showAnimate" v-on:click="audioCall('{{ $webphone->callerId }}' , '{{ $webphone->name }}')">
                    ...
                    <div v-if="status.show" class="flex mx-auto max-w-md mr-auto bg-gray-600 text-white p-5 rounded-md mt-4" :class="{ 'animate__animated animate__backInLeft' : status.showAnimate , 'animate__animated animate__backOutLeft' : !status.showAnimate  }">
                        ...
                        <button v-on:click="cancelRejectBy" class="bg-red-600 rounded-md p-3 absolute inset-y-0 right-0 hover:bg-red-500">End Call</button>
                    </div>
    ...
    <!-- moment js  -->
    <script type="text/javascript" src="{{ asset('js/moment-with-locales-2.24.0.min.js') }}"></script>

    <!-- sipjs  -->
    <script type="text/javascript" src="{{ asset('js/sip-0.11.6.min.js') }}"></script>

    {{-- some path used in js file --}}
    <script>
        let wssServer     = "{{ config('app.WssServer') }}"
        let WebSocketPort = "{{ config('app.WebSocketPort') }}"
        let SipUsername   = "{{ config('app.SipUsername') }}"
        let SipPassword   = "{{ config('app.SipPassword') }}"

        let RingtoneSrc = "{{ asset('media/Ringtone_1.mp3') }}"
        let EarlyMedia_EuropeanSrc = "{{ asset('media/Tone_EarlyMedia-European.mp3') }}"
    </script>

    <!-- voipiran phone -->
    <script src="{{ asset('js/new-phone.js') }}"></script>
</body>
